...
from app.models.schedule import ClassSession, StudentAttendance
...
@router.get("/{student_id}", response_model=ResponseBase)
async def get_student_schedule(
    ...
    query = db.query(ClassSession).join(
        StudentAttendance, ClassSession.id == StudentAttendance.session_id
    ).filter(
